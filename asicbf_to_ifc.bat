...
echo =======================================================
echo     asicbf_to_ifc Launcher (MSVC v142 Multi-Config)
echo =======================================================
...
echo --- Launching asicbf_to_ifc.exe (%CONFIG%) ---
if not exist "%APP_BIN%\asicbf_to_ifc.exe" (
    echo.
    set "APP_BIN=%ROOT_DIR%build_v142\bin\%CONFIG%"
)
if not exist "%APP_BIN%\asicbf_to_ifc.exe" (
    echo.
    echo [ERROR] Cannot find "%APP_BIN%\asicbf_to_ifc.exe"!
    echo Please compile the 'asicbf_to_ifc' target in "%CONFIG%" mode first.
    echo.
    pause
    exit /b 1
)

echo Running: "%APP_BIN%\asicbf_to_ifc.exe" %ARGS%
"%APP_BIN%\asicbf_to_ifc.exe" %ARGS%
set "EXIT_CODE=%ERRORLEVEL%"

echo.
echo =======================================================
echo  asicbf_to_ifc execution finished with code: %EXIT_CODE%
echo =======================================================
pause
