...
namespace {
    ...
    static void md5Transform(uint32_t state[4], const uint8_t block[64]) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            f += a + K[i] + M[g];
            a = d; d = c; c = b; b += leftRotate(f, S[i]);
        }
    ...
    static std::string md5(const std::string& input) {
        ...
        if (padLen == 128) md5Transform(ctx.state, finalBlock + 64);
                            ...
                            const std::string& contentType = "application/json")
{
    ...
    HINTERNET hSession = WinHttpOpen(L"GeometryService/1.0",
...
GeometryService::ExtractGeoParams(const Handle(BrNode_adObject)& adObj)
{
    ...
    for (int i = 1; i <= psets.Length(); ++i) {
        ...
        for (int j = 1; j <= props.Length(); ++j) {
            ...
            // 根据值类型转换
            if (vtype == "Real") {
                try { result.params[key] = std::stod(val); }
                catch (...) { result.params[key] = val; }
            } else if (vtype == "Int") {
                try { result.params[key] = std::stoi(val); }
                catch (...) { result.params[key] = val; }
            } else {
                                                ...
                                                const json& params)
{
    ...
    json sorted = params;
    std::string input = modelType + "|" + sorted.dump(-1, ' ', false, json::error_handler_t::replace);
    return md5(input);
}
                                      ...
                                      const json& params)
{
    ...
    result.brepData = std::string(response.data() + 4 + jsonLen,
                                   response.size() - 4 - jsonLen);
...
// ------ 创建几何定义节点 ------
Handle(BrNode_adGeometricDef)
GeometryService::CreateGeoDef(const std::string& paramGeoId,
                               const json& allParams,
                               const TopoDS_Shape& shape)
{
    Handle(BrNode_adGeometricDef) geoDef = m_model->AddadGeometricDef();
    if (geoDef.IsNull()) return nullptr;
...
// ------ 回写参数 ------
void GeometryService::WriteBackParams(const Handle(BrNode_adPropertySet)& geoPset,
                                       const json& returnedParams)
{
    if (geoPset.IsNull()) return;

    NCollection_Sequence<Handle(BrNode_adProperty)> props = geoPset->GetPropertiesList();
    for (int j = 1; j <= props.Length(); ++j) {
        Handle(BrNode_adProperty) p = props.Value(j);
        if (p.IsNull()) continue;
...
GeometryService::BuildGeometry(const Handle(BrNode_adObject)& adObj)
{
    ...
    Handle(BrNode_adGeometricDef) geoDef = CreateGeoDef(paramGeoId, sr.allParams, shape);
                                          ...
                                          const json& inputParams)
{
    ...
    if (!sr.success) {
        std::cerr << "[GeometryService] BuildFromParams failed: " << sr.error << std::endl;
        return result;
    }
