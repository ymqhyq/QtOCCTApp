...
#pragma warning(disable: 4275)
			...
			public:
				topology_error();
				topology_error(const char* const s);
...
#pragma warning(pop)
			...
			public:
				...
				inline uint32_t identity() const { return identity_; }
			};
			...
			struct IFC_GEOM_API eigen_base {
				...
				inline eigen_base(const eigen_base& other) {
					this->components_ = other.components_ ? new T(*other.components_) : nullptr;
				}

				inline eigen_base(const T& other) {
					this->components_ = new T(other);
				}

				inline eigen_base& operator=(const eigen_base& other) {
					if (this != &other) {
						this->components_ = other.components_ ? new T(*other.components_) : nullptr;
					}
					return *this;
				...
				inline uint32_t hash_components() const {
			...
			public:
				...
				virtual ~matrix4();
				matrix4(const Eigen::Matrix4d& c);
				matrix4(const Eigen::Vector3d& o, const Eigen::Vector3d& z, const Eigen::Vector3d& x);
				matrix4(const Eigen::Vector3d& o, const Eigen::Vector3d& z);

				bool is_identity() const;

				void print(std::ostream& o, int indent = 0) const;

				virtual matrix4* clone_() const;
				inline virtual kinds kind() const { return MATRIX4; }
				...
				Eigen::Vector3d translation_part() const;
			};
			...
			struct IFC_GEOM_API colour : public item, public eigen_base<Eigen::Vector3d> {
				...
				virtual colour* clone_() const;
				inline virtual kinds kind() const { return COLOUR; }
			...
			struct IFC_GEOM_API style : public item {
				...
				virtual style* clone_() const;
				inline virtual kinds kind() const { return STYLE; }

				virtual size_t calc_hash() const;

				// @todo equality implementation based on values?
				inline bool operator==(const style& other) const { return instance == other.instance; }
